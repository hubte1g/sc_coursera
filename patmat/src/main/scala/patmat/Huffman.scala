...
object Huffman {
  ...
  // weight is duplicated in both case call constructors
  // could be pushed up into the abstract class
  ...
  // Given: 2 CodeTrees are combined using the Fork case class
   ...
   *   }
   */
  // iterate through the list of chars and use a HashMap to keep track Characters
  // and counts/Char. Problem can be simplified first going through the List using
  // recursive calls with an accumulator as a function parameter to track state.
  // Chars are then printed to make sure it was traversed correctly.
   ...
   */
    def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
      case left :: right :: chars => (makeCodeTree(left, right) :: chars)
   ...
   */
    def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
      ...
      def merger(x: Int, p: (Char, List[Bit])): (Char, List[Bit]) = p match {
        case (c, bs) => (c, x :: bs)
      }
      a.map(merger(0, _)) ::: b.map(merger(1, _))
    }
